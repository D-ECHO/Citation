...
var JetEngine = {
	new : func(eng_num) {
		...
		m.reverser_pos =      props.globals.initNode("fdm/jsbsim/propulsion/engine["~eng_num~"]/reverser-angle-rad",0,"DOUBLE");
	...
	update : func{
		...
		# fluid pump factor
		var factor = 1.0 - (me.n2.getValue() * 0.01);
		# oil pump
		var x = me.n2.getValue() * 0.77;
...
setlistener("/sim/signals/fdm-initialized", func {
	...
	if (getprop("/autopilot/heading_overlay")) {
		...
		if (overlay_name != nil) {
			...
			print ("Setting passive-mode FALSE because /autopilot/heading-overlay is nonzero");
...
var update_systems = func() {
	...
	if(getprop("controls/flight/speedbrake")>0) {
		if(getprop("engines/engine[0]/n2")>85
		or getprop("engines/engine[1]/n2")>85) {
...
var pilot_hsi_listener = setlistener ("/instrumentation/hsi[0]/selected-nav", func (selected_nav) {
...
var copilot_hsi_listener = setlistener ("/instrumentation/hsi[1]/selected-nav", func (selected_nav) {
